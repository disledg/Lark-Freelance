...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $result = query("SELECT * FROM users WHERE email = '$email' AND role = 'developer'");
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        if (password_verify($password, $user['password'])) {
            if ($user['status'] === 'approved') {
                ...
                addLog($user['id'], 'login', 'Developer logged in');
...
<head>
    ...
    <title>Вход для разработчиков | Lark Freelance</title>
